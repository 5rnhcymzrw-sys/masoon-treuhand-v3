...
if (form) {
  form.querySelectorAll('input:not(.honey),textarea').forEach(field => {
    field.addEventListener('invalid', () => { field.setCustomValidity(field.validity.typeMismatch ? 'Bitte geben Sie eine gültige E-Mail-Adresse ein.' : 'Bitte füllen Sie dieses Feld aus.'); });
  ...
  form.addEventListener('submit', async event => {
    ...
    try {
      const response = await fetch('https://api.web3forms.com/submit', { method: 'POST', headers: { 'Content-Type': 'application/json', Accept: 'application/json' }, body: JSON.stringify(data), signal: AbortSignal.timeout(20000) });
